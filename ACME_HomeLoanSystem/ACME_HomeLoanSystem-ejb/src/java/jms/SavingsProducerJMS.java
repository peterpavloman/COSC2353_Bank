...
public class SavingsProducerJMS implements SavingsProducerBridge
{
	...
	public SavingsProducerJMS() throws JMSException, NamingException
	{
		...
		mSession = mConnection.createSession(true,
	...
	public void close()
	{
		// Best effort attempt to close these resources
		try
		{
			mProducer.close();
			mSession.close();
			...
			BigDecimal aAmount) throws ApplicationLogicException
	{
		try
		{
			...
			// sendJMSRequestNoResponse(lMessage);
			// return true;
		}
	...
	private void sendJMSRequestNoResponse(ObjectMessage aMessage) throws JMSException
	{
		long lCorrelationIdCounter = mCorrelationIdCounter;
		mCorrelationIdCounter++;
		String lCorrelationId = Long.toHexString(lCorrelationIdCounter);
	...
	public BigDecimal getTotalSavingsBalance() throws ApplicationLogicException
	{
		try
		{
			...
			lMessage = MessageProtocolSavings.GetTotalSavingsBalance.
					formatRequestMessage(lMessage);
			...
			{
				return MessageProtocolSavings.GetTotalSavingsBalance.
						getResponseBalance(lResponse);
			}
		}
		catch (JMSException aException)
